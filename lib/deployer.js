...
module.exports = function(args) {
  ...
  const deployRepoDir = pathFn.join(baseDir, '.deploy_git');
  const deployFileDir = pathFn.join(deployRepoDir, args.repo_public_dir);
  ...
  // console.info('hexo-deployer: ', args);
  // console.info('hexo-deployer: ', this);
  ...
  function setup() {
    ...
    return fs.writeFile(pathFn.join(deployRepoDir, 'placeholder'), '').then(() => {
      return git('init');
    }).then(() => {
  ...
  return fs.exists(deployRepoDir).then(exist => {
    if (exist) return;
    ...
    return fs.emptyDir(deployRepoDir);
  }).then(() => {
    const opts = {};
    ...
    log.info('hexo-deployer: publicDir=' + publicDir + '  deployFileDir=' + deployFileDir, opts);
    return fs.copyDir(publicDir, deployFileDir, opts);
  }).then(() => {
    ...
    const mapFn = function(dir) {
      ...
      const extendDist = pathFn.join(deployFileDir, dir);
      ...
      log.info('hexo-deployer: extendPath=' + extendPath + '  extendDist=' + extendDist, opts);
    ...
    log.i('hexo-deployer:  ', repo);
    return push(repo);
  });
};

function commitMessage(args) {
  const message = args.m || args.msg || args.message || 'update posts.';
